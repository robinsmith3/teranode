...
def select_config_directory(project_name: str) -> str:
    """Let user select a timestamped config directory within configs/project_name"""
    project_dir = os.path.join("configs", project_name)
    if not os.path.exists(project_dir):
        print(f"No '{project_dir}' directory found. Please download configs first.")
        return None
    
    subdirs = [d for d in os.listdir(project_dir) if os.path.isdir(os.path.join(project_dir, d))]
    if not subdirs:
        print(f"No timestamped subdirectories found in '{project_dir}'. Please download configs first.")
        return None
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(subdirs):
                return os.path.join(project_dir, subdirs[choice-1])
            print("Invalid selection. Try again.")
        except ValueError:
            print("Please enter a number.")

def upload_configs(server_url: str, project_id: str, project_name: str, nodes: List[gns3fy.Node], do_reload: bool):
    """Upload configurations from a selected configs/project_name/timestamp directory using GNS3 API"""
